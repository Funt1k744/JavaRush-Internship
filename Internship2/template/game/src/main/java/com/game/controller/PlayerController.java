...
public class PlayerController {
    ...
    public ResponseEntity<Player> createPlayer(@RequestBody Player player) {
        ...
        final Integer experienceNextLevel = playerService.computeExperienceNextLevel(level, player.getExperience());
        player.setUntilNextLevel(experienceNextLevel);
    ...
    public ResponseEntity<Player> getPlayer(@PathVariable(value = "id") String pathId) {
        final Long id = convertIdLong(pathId);
        if (id == null || id <= 0) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
    ...
    private Long convertIdLong(String pathId) {
        if (pathId == null) {
            return null;
        } else try {
